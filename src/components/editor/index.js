...
function processEditorContent(root) {
  ...
  if (root instanceof HTMLTextAreaElement && root.matches(EDITOR_SELECTOR)) editors.push(root)
...
function initCharacterCounter(textarea) {
  ...
  const update = () => {
    ...
    if (value) value.textContent = String(textarea.value.length).padStart(2, '0')
  }
...
function getLegacyDraftStorageKey(textarea) {
  const url = new URL(window.location.href)
  const topicId = url.searchParams.get('id') || url.searchParams.get('tid') || url.searchParams.get('topic')
...
function initDiceButton(root) {
  const toolbars = []
  if (root instanceof Element && root.id === 'form-buttons') toolbars.push(root)
  ...
  toolbars.forEach((toolbar) => {
    ...
    button.addEventListener('click', () => {
      const textarea = toolbar.closest('form')?.querySelector(EDITOR_SELECTOR) || document.querySelector(EDITOR_SELECTOR)
...
function initExtraColors(root) {
  const colorAreas = []
  if (root instanceof Element && root.id === 'color-area') colorAreas.push(root)
  colorAreas.push(...root.querySelectorAll('#color-area'))

  colorAreas.forEach((colorArea) => {
    if (colorArea.querySelector('[data-forum-extra-colors], td[style*="thistle"]')) return
...
function replaceDiceCodes(postContent) {
  ...
  while (walker.nextNode()) {
    ...
    if (textNode.parentElement?.closest('code, pre, textarea, .dice-result')) continue
...
function createDiceResult(postContent, match) {
  const encodedRolls = match[1].replace(/—/g, '-').split('-').filter(Boolean).map(Number)
  ...
  const rolls = decodeDiceRolls({ postContent, encodedRolls, sideCount, isNewScheme })
  ...
  heading.textContent = t('editor.dice.rollDescription', { count: diceCount, sides: sideCount })
  paragraph.append(heading, document.createElement('br'), document.createElement('br'))
  paragraph.append(t('editor.dice.result', { rolls: rolls.join(' + '), total }))
  quote.append(paragraph)
  result.append(quote)
  return result
}

function decodeDiceRolls({ postContent, encodedRolls, sideCount, isNewScheme }) {
  ...
  return encodedRolls.map((encodedRoll) => {
    ...
    randomState =
      (((randomState >> 1) + ((randomState & 1 ? 0 : 1) << 23) + value + seconds) ^
        postNumber) &
      0xffffff
...
function getPostNumber(postContent) {
  const identifier = postContent.closest('.post[id]')?.id || postContent.id || ''
  return Number(identifier.match(/\d+/)?.[0] || 0)
}

function getPostSeconds(postContent) {
  const post = postContent.closest('.post')
  const timestamp = Number(postContent.dataset.posted || post?.dataset.posted || 0)
